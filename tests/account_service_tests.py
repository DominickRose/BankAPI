...
def test_2_retrieve_client_accounts1():
    results = accountService.retrieve_all_client_accounts(1)
    assert len(results) == 3

def test_3_retrieve_client_accounts2():
    results = accountService.retrieve_all_client_accounts(2)
    assert len(results) == 2

def test_4_retrieve_accounts_in_range1():
    results = accountService.retrieve_all_client_accounts_in_range(1, 150, 250)
    assert len(results) == 2

def test_5_retrieve_accounts_in_range2():
    results = accountService.retrieve_all_client_accounts_in_range(1, 0, 50)
    assert len(results) == 0

def test_5_1_retrieve_specific_account():
    result = accountService.retrieve_specific_account_for_client(1, test_account.account_id)
...
def test_7_delete_specific_account():
    ...
    assert len(accountService.retrieve_all_client_accounts(1)) == 2 #Prove deletion
...
def test_10_change_money1():
    ...
    assert accountService.retrieve_specific_account_for_client(1, 1).balance == 150

def test_11_change_money1():
    accountService.change_money_in_account(1, 1, -75)
    assert accountService.retrieve_specific_account_for_client(1, 1).balance == 75
...
def test_14_transfer_funds():
    ...
    account1 = accountService.retrieve_specific_account_for_client(1, 1)
    account2 = accountService.retrieve_specific_account_for_client(1, 2)
